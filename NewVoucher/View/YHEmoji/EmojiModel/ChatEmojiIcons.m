...
+ (NSArray *)emojis {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (NSInteger i=1; i <=kEmojisCount; i++) {
            NSString *key = [NSString stringWithFormat:@"[em_%@]",@(i)];
            NSString *emojiImgName = [emojiDic valueForKey:key];
            [array addObject:emojiImgName];
