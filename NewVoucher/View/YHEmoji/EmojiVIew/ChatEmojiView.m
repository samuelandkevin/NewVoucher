...
- (instancetype)initWithFrame:(CGRect)frame{
    CGRect _frame_ = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, ChatEmojiView_Hight);
...
- (void)setFrame:(CGRect)frame{
    if ((frame.size.height == ChatEmojiView_Hight) && (frame.size.width ==[UIScreen mainScreen].bounds.size.width)) {
...
//    [bottom addSubview:panda];
    ...
    sendB.frame = frame;
    sendB.backgroundColor = [UIColor colorWithRed:0/255.0 green:191/255.0 blue:143/255.0 alpha:1.0];
...
- (void)sender:(IconButton*)sender{
    if (sender) {
        ...
        selectIcon = sender;
        selectIcon.backgroundColor = [UIColor colorWithRed:229/255.0 green:229/255.0 blue:229/255.0 alpha:1.0];
    }
}
...
- (void)addEmjioWith:(Class)class{
    ...
    CGFloat space = ([UIScreen mainScreen].bounds.size.width - EmojiView_Border*2.0f-count_lins*EmojiIMG_Width_Hight)/(count_lins-1);
    scroll.contentSize = CGSizeMake(page_support*[UIScreen mainScreen].bounds.size.width, 0);
    ...
    for (int i = 0; i < page_support; i++) {
        ...
        for (int j = 0; j < array.count - 1; j++) {
            ...
            CGRect frame = CGRectMake(EmojiView_Border+list_w*(space+EmojiIMG_Width_Hight)+(i*[UIScreen mainScreen].bounds.size.width), EmojiView_Border+lins_w*(EmojiIMG_Space_UP+EmojiIMG_Width_Hight), EmojiIMG_Width_Hight, EmojiIMG_Width_Hight);
...
//            [button addGestureRecognizer:loTap];
        ...
        EmojiButton * del_B = [[EmojiButton alloc]initWithFrame:CGRectMake(EmojiView_Border+(count_lins-1)*(space+EmojiIMG_Width_Hight)+(i*[UIScreen mainScreen].bounds.size.width), EmojiView_Border+(EmojiIMG_Lines-1)*(EmojiIMG_Space_UP+EmojiIMG_Width_Hight), EmojiIMG_Width_Hight, EmojiIMG_Width_Hight)];
